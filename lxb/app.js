...
function reshootGate(bad) {
  ...
  if (!bad.length) {
    if (!passengers.length && backSides.length) {  // toàn mặt sau, chưa có khách nào
      ...
      const n = passengers.length; // hoàn thành → xanh lá + bộ đếm N/N
      el.style.background = '#dcfce7'; el.style.border = '1px solid #16a34a'; el.style.color = '#166534';
      el.innerHTML = `✅ <b>Đủ ${n}/${n} khách</b> — tất cả ảnh đã đọc đủ 4 thông tin. Có thể Xuất file.` + backHtml;
    }
    return;
  }
  const nums = bad.map(p => passengers.indexOf(p) + 1).join(', ');
  const done = passengers.length - bad.length; // bộ đếm: đã đủ / tổng
  el.style.background = '#fef2f2'; el.style.border = '1px solid #dc2626'; el.style.color = '#991b1b';
  el.innerHTML = `⚠️ <b>Đủ ${done}/${passengers.length} khách</b> — còn ${bad.length} ảnh chưa đọc đủ (dòng ${nums} tô đỏ). ` +
